...
export default function Analytics(){
  ...
  const [filters, setFilters] = useState({ range:'30d', scope:'all' })
  useEffect(()=>{ (async()=>{ try{ setData(await apiFetch('/analytics/overview')) } catch{} })() }, [filters])
    ...
    <div style={{ display:'grid', gap:12 }}>
      <div className="panel" style={{ display:'flex', gap:8, alignItems:'center', justifyContent:'space-between' }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          <a href="/api/analytics/export.csv" className="kpi-title" style={{ padding:'6px 10px', border:'1px solid var(--border)', background:'var(--surface)', borderRadius:8, textDecoration:'none' }}>{t('pages.analytics.actions.export_csv')||'Export CSV'}</a>
          <a href="/api/analytics/export.json" className="kpi-title" style={{ padding:'6px 10px', border:'1px solid var(--border)', background:'var(--surface)', borderRadius:8, textDecoration:'none' }}>{t('pages.analytics.actions.export_json')||'Export JSON'}</a>
        </div>
      </div>
