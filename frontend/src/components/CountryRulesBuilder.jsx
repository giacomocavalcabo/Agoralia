  ...
  return (
    <div className="panel" style={{ display:'grid', gap:8 }}>
      <table className="table">
        <thead><tr><th>ISO</th><th>{t('import.legal.disclosure')}</th><th>Quiet hours</th><th>DNC</th><th></th></tr></thead>
        <tbody>
          {rows.map((r, i) => (
            <tr key={i}>
              <td><input className="input" value={r.iso} onChange={(e)=> update(i, { iso: e.target.value.toUpperCase() })} placeholder="IT" style={{ width:80 }} /></td>
              <td><input className="input" value={r.disclosure} onChange={(e)=> update(i, { disclosure: e.target.value })} placeholder={t('import.legal.disclosure')} /></td>
