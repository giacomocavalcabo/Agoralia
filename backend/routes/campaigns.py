...
async def get_campaign(request: Request, campaign_id: int) -> Dict[str, Any]:
    ...
    with tenant_session(request) as session:
        ...
        return {
            ...
            "quiet_hours_enabled": bool(c.quiet_hours_enabled) if hasattr(c, 'quiet_hours_enabled') and c.quiet_hours_enabled is not None else None,
            "quiet_hours_weekdays": getattr(c, 'quiet_hours_weekdays', None),
            "quiet_hours_saturday": getattr(c, 'quiet_hours_saturday', None),
            "quiet_hours_sunday": getattr(c, 'quiet_hours_sunday', None),
            "quiet_hours_timezone": getattr(c, 'quiet_hours_timezone', None),
            "created_at": c.created_at.isoformat(),
